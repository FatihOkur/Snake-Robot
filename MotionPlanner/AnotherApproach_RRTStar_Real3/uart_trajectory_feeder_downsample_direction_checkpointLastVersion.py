...
# ──────────────────────────────────────────────────────────────────
# INVERSE KINEMATICS — Nose ➜ Seg3 (Anchor / J3)
# ──────────────────────────────────────────────────────────────────
# Segment lengths in map units  (1 unit = 10 cm)
L_HEAD = 2.5     # Nose  → J1
L_SEG2 = 3.05    # J1    → J2
L_SEG3 = 3.65    # J2    → J3 (Anchor)


def calculate_j3_from_nose(nose_x, nose_y, nose_yaw_rad, q1_deg, q2_deg):
    """
    Walk backwards along the robot's kinematic chain from the
                ...
                if "REQ" in incoming_str:
                    ...
                    if step_index > 0:
                        ...
                        if is_fresh:
                            ...
                            # est_x/est_y now represent the *Nose*, but
                            # base_coordinates track the Seg-3 Anchor.
                            # Use Inverse Kinematics to project from
                            # Nose back to Anchor before comparing.
                            ...
                            if "base_coordinates" in prev_step:
                                ...
                                pos_error = math.hypot(j3_x - exp_x, j3_y - exp_y)
                                print(f"[NAV] IK Anchor est: ({j3_x:.2f}, {j3_y:.2f})  "
