...
def main():
    ...
    prev_cmd = None  # the raw cmd of the previous iteration (for flushing)

    for i, cmd in enumerate(raw_trajectory):
        dh, dl = get_dist(cmd)

        # --- DIRECTION-CHANGE FLUSH ---
        # If the running accumulation is moving one way and THIS raw step moves
        # the opposite way, flush what we have FIRST (using the previous step's
        # angles), so forward and reverse motion never cancel inside one packet.
        # This keeps the back-and-forth oscillation intact at the ~2 reversal
        # points in the path. Everywhere else it never triggers, so the servo
        # deadband merging below is completely unaffected.
        head_reversal  = (acc_head  * dh < -1e-12)   # strict opposite signs
