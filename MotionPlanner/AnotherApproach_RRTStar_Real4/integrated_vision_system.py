...
# ==========================================
# FIXED LOCALIZATION-REFERENCE MARKERS
# (NOT the planner goal)
# Coordinates MUST be in the MotionPlanner/AnotherApproach_RRTStar_Real4
# map frame (complex_map, 55x85, 1 unit = 10 cm).
# ==========================================
...
# Calibration: Adjust so map_yaw reads +pi/2 when head faces map +y.
CAMERA_YAW_OFFSET = math.pi / 2  

# Forward distance from camera lens to physical nose tip on the Head segment.
LENS_TO_NOSE_OFFSET_M = 0.05
...
while True:
    ...
    if ids is not None:
        ...
        for i in range(len(ids)):
            ...
            # ── Nose Correction ──────────────────────────────────────
            # The camera lens sits behind the physical Nose on the Head
            # segment. Subtract that offset from the forward-depth axis 
            # so the reported position corresponds to the Nose rather than the lens.
            z_m -= LENS_TO_NOSE_OFFSET_M
            marker_id = ids[i][0]
            ...
            if marker_id in CHECKPOINT_LOCATIONS:
                ...
                # The camera is robot-mounted looking at fixed markers.
                # Compute robot/head heading from the inverse transform (camera-in-marker).
                rmat_inv = rmat.T
                yaw_relative = math.atan2(rmat_inv[1, 0], rmat_inv[0, 0])
                map_yaw = yaw_relative + CAMERA_YAW_OFFSET
                
                # Rotate the camera-frame offset by the robot's map yaw.
                # Camera +z is forward (along yaw), +x is right (yaw - pi/2).
                offset_x_map = (z_m * math.cos(map_yaw) - x_m * math.sin(map_yaw)) * MAP_SCALE
                offset_y_map = (z_m * math.sin(map_yaw) + x_m * math.cos(map_yaw)) * MAP_SCALE
                
                robot_absolute_x = loc['x'] - offset_x_map
                robot_absolute_y = loc['y'] - offset_y_map

                # [ASSERTION] This system assumes the ArUco reference is rigidly 
                # attached to the HEAD segment, so the broadcast pose IS the head pose
                # (x_head, y_head, yaw_head).
