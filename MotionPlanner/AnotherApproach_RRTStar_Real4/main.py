...
def calculate_straight_state_from_head(head_x, head_y, yaw_deg):
    """
    Build a straight (all joints = 0) state from the head pose.
    In the head-anchored model, the first three state entries ARE the head pose.
    """
...
def main():
    ...
    if replan_mode:
        # --- REPLAN MODE: start from the robot's current head pose ---
        # The feeder now sends head coordinates directly (no J3 conversion).
        START_STATE = np.array([
            args.start_x, args.start_y, args.start_yaw_rad,
            args.start_q1, args.start_q2, args.start_q3,
        ])
        print(f"[REPLAN] Start state from feeder (head pose): {START_STATE}")
    else:
    ...
    for i, state in enumerate(anim_frames):
        ...
        if prev_state is None:
            ...
            body_old = SnakeRobotModel.get_body_from_head_base(prev_state)
            body_new = SnakeRobotModel.get_body_from_head_base(state)
            
            # Segment 1 (Head) distance — tracked at body[1] (J1)
            ...
            # Segment 3 (Link 2) distance — tracked at body[3] (J3)
            j3_old_x, j3_old_y = body_old[3]
            j3_new_x, j3_new_y = body_new[3]
            dist_link2 = math.hypot(j3_new_x - j3_old_x, j3_new_y - j3_old_y)
            
            # Determine the direction by checking the dot product of the movement
            # vector against the base's heading (yaw_head)
    ...
    def draw_ghost(s, c):
        b = SnakeRobotModel.get_body_from_head_base(s)
        bx, by = zip(*b)
