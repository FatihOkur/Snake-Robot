...
class TailBaseRRT:
    ...
    def run_local_controller(self, start_node):
        ...
        MAX_STEPS = 150  # Increased to give it time to precisely maneuver
        steps = 0
        ...
        while steps < MAX_STEPS:
            ...
            if dist < 0.2 and joint_diff < 2.0:
            ...
            # ANTI-ORBITING FIX: Dampen curvature when very close to prevent violent whipping
            if dist < 1.0:
                curvature *= dist 
            ...
            # Use smaller steps for the precision docking phase
            step_len = min(0.5, dist)
            ...
            new_state[3:5] += np.clip(joint_error_front, -SAFE_JOINT_STEP, SAFE_JOINT_STEP)
                    
            # Tail joint
            q3_error = q3_ideal_deg - state[5]
            new_state[5] += np.clip(q3_error, -SAFE_JOINT_STEP, SAFE_JOINT_STEP)
            new_state[3:] = np.clip(new_state[3:], -config.JOINT_LIMIT, config.JOINT_LIMIT)
    ...
    def step(self):
        ...
        if SnakeRobotModel.is_valid_state(new_state, self.env):
            ...
            if d_pos < config.GOAL_POS_TOLERANCE and yaw_diff < np.deg2rad(config.GOAL_ANGLE_TOLERANCE):
                ...
                # If docking succeeds perfectly, we are done!
